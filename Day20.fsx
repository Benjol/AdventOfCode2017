//For some reason AoC doesn't accept any of the answers that come out of this code (142 or 301)
// for this reason I include my input below the code:
...
let manhattan (a,b,c) = sqrt (a ** 2. + b ** 2. + c ** 2.)

//First attempt: lowest acceleration, in case of a tie, sort by velocity (gives 301 for my input)
...
let runtime = particles |> Array.Parallel.map timetostable |> Array.max
...
// I thought this would give the definitive answer, but it apparently doesn't - this says 301
particles |> Array.indexed |> Array.Parallel.map (fun (i,p) -> i, futurepos p runtime) |> Array.minBy (fun (i,p) -> manhattan p) |> fst
// But running these says 142
...
//Re-run, gives 301, same as my analytic version above
...
//Re-run, gives 142, same as my analytic version above
Seq.unfold (fun p -> Some(p |> Array.indexed |> Array.minBy (fun (i,p) -> manhattan p.P) |> fst, p |> Array.map tick)) particles |> Seq.skip 1000 |> Seq.take 10 |> Seq.toList

let input2 = @"p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>
